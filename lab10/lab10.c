...
void *condProducer(void* arg) {
	...
	// Locking mutex before accessing buffer.
	...
	// Adding an element to the buffer ("do_part_1();")
	insert(cq->q, in);
	
	// Signaling that item added to buffer 
	pthread_cond_signal(cq->cond);//
	// unlocking mutex after modifying buffer:
...
void *condConsumer(void* arg) {
	...
	// Locking mutex before we access the buffer.
	pthread_mutex_lock(cq->mutex);

	// We need to wait until there is an item in the buffer:
	while (cq->q->index == 0) {	// "if (part_1_not_done) {""
		pthread_cond_wait(cq->cond, cq->mutex);
	}
	// Remove an element from the buffer.
	condTotal += extract(cq->q); 	// "do_part_2();"

	// Now we unlock mutex after modifying buffer
	pthread_mutex_unlock(cq->mutex);
